...
int **alloc_matrix(int line, int coloumn) {
	int **a = calloc(line, sizeof(int *)); 
    if (!a) {
		fprintf(stderr, "malloc() failed\n");
		return NULL;
	}
	for (int i = 0; i < line; i++) {
		a[i] = calloc(coloumn, sizeof(int)); 
        if (!a[i]) {
			fprintf(stderr, "malloc() failed\n");
			while (--i >= 0) {
				free(a[i]);
...
mg_create(int nodes)
{
    matrix_graph_t *grf = malloc(sizeof(matrix_graph_t));
    if (!grf)
        return NULL;
    
    grf->matrix = alloc_matrix(nodes, nodes);
    grf->nodes = nodes;
    return grf;
}
...
void dfs_matrix_graph(matrix_graph_t* mg, int node) {
    stack *stack = st_create(sizeof(ll_node_t));
    int* visited = (int*) calloc(mg->nodes, sizeof(int));
    ...
    while (!st_is_empty(stack)) {
        ...
        // Parcurgem vecinii nodului curent in ordinea matricei
        for (int i = 0; i < mg->nodes; i++) {
            if (mg->matrix[current][i] && !visited[i]) {
                st_push(stack, &i);
                visited[i] = 1;
            }
        }
