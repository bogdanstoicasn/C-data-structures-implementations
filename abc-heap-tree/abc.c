...
void bst_tree_insert(bst_tree_t *bst_tree, void *data)
{
    ...
    while (root != NULL) {
        parent = root;
        int cmp_result = bst_tree->cmp(data, root->data);
        if (cmp_result < 0) {
            root = root->left;
        } else {
            root = root->right;
    ...
    // bagam la copcchi
    int cmp_result = bst_tree->cmp(data, parent->data);
    if (cmp_result < 0) {
        parent->left = node;
    } else {
        parent->right = node;
...
static bst_node_t *bst_minimum_node(bst_node_t *bst_node)
{
    if (!bst_node)
        return NULL;
...
static bst_node_t *__bst_tree_remove(bst_node_t *bst_node,
    ...
    if (rc < 0) {
        ...
        if (!bst_node->left && !bst_node->right) {
            ...
            tmp = bst_minimum_node(bst_node->right);
            bst_node->data = malloc(data_size);
