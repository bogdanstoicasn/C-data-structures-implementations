...
static void __heap_pop_fix(heap_t *heap, int pos)
{
    ...
    while (l < heap->size) {
        int max_child = (r < heap->size && heap->cmp(heap->arr[r], heap->arr[l]) < 0) ? r : l;

        if (heap->cmp(heap->arr[max_child], heap->arr[p]) < 0) {
            // interschimb
            tmp_team = heap->arr[p];
            heap->arr[p] = heap->arr[max_child];
            heap->arr[max_child] = tmp_team;
